  ...
  <div class="products-recommond">
    <div class="weui-flex title-box">
      <p class="weui-flex__item">
        <span class="left-border"></span>{{bossTitle[2].title}}</p>
      ...
      <li class="wonderful_list" v-for="(item,index) in datas" @click="detail(item)">
        <a href="javascript:;" class="weui-flex">
          <div class="list_left">
            ...
            <div class="bg-img reading-box" v-if="item.type == 0"></div>
            <div class="bg-img audio-box" v-if="item.type == 1"></div>
            <div class="bg-img video-box" v-if="item.type == 2"></div>
          </div>
          <div class="list_right1">
            ...
            <!--如果类型是0，点击内容描述时候，就跳转到视频详情页-->
              ...
              <span class="free weui-flex__item" v-if="item.price == 0">免费</span>
...
<script type="text/ecmascript-6">
  ...
  export default {
    ...
    mounted() {
      this.$http.get(globle.url+'getArticlesById/'+this.bossTitle[2].id+'/2?token='+globle.tokens).then(function(res){
        this.datas = res.data;
        console.log(res.data);
        this.topicId=this.bossTitle[2].id;
      },function(res){
        console.log(res)
      })
    },
